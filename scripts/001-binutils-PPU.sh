...
if [ ! -d ${BINUTILS} ]; then
  ...
  if [ ! -f ${BINUTILS}.tar.bz2 ]; then wget -q --show-progress --continue https://ftpmirror.gnu.org/gnu/binutils/${BINUTILS}.tar.bz2; fi

  ## Download an up-to-date config.guess and config.sub
  if [ ! -f config.guess ]; then wget -q --show-progress --continue https://git.savannah.gnu.org/cgit/config.git/plain/config.guess; fi
  if [ ! -f config.sub ]; then wget -q --show-progress --continue https://git.savannah.gnu.org/cgit/config.git/plain/config.sub; fi
